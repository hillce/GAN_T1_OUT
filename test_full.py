...
knNums = list(itertools.combinations([5,6],2))
...
with torch.no_grad():
    ...
    for i,data in enumerate(loaderTest):
        ...
        for knCnt,kn in enumerate(knNums):
            sys.stdout.write("\rSubj: [{}/{}], Kn [{}/{}]".format(i+1,testLen,knCnt,len(knNums)))
            ...
            # print(reconNumpy.shape,imgs.shape)

            reconNumpyT1 = reconT1.numpy()

            # for ii in range(reconNumpy.shape[0]):
            #     for jj in range(reconNumpy.shape[1]):
            #         grid[kn[0],kn[1],jj] += (np.sum(abs(reconNumpy[ii,jj,:,:]-imgs[ii,jj,:,:]))/(reconNumpy.shape[2]*reconNumpy.shape[3]))
            ...
            for i in range(reconNumpy.shape[1]):
                ...
                im = ax[3,i].imshow(perErrMap[0,i,:,:],cmap="jet",vmax=20)
            ...
            im = ax[0,0].imshow(gtT1Numpy[0,:,:,:],vmin=500,vmax=900)
            fig.colorbar(im,ax=ax[0,0])
            ax[0,0].set_title("Original T1")
            ax[0,0].set_xticks([])
            ax[0,0].set_yticks([])

            im = ax[1,0].imshow(reconNumpyT1[0,:,:,:],vmin=500,vmax=900)
            ...
            im = ax[1,1].imshow(perErrMapT1[0,:,:,:],cmap="jet",vmax=20)
